...
@section('content')
    <div class="space-y-6">
        <div class="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
            ...
            <form action="{{ route('reports.accountant.generate') }}" method="POST"
                ...
                <div class="flex items-end gap-2">
                    ...
                    {{-- Botón PDF con todos los parámetros actuales --}}
                    <a href="{{ route('reports.accountant.download.pdf', request()->all()) }}"
                    ...
                    {{-- Botones para Exportación CSV --}}
                    <div class="flex gap-1">
                        <a href="{{ route('reports.accountant.download.csv', array_merge(request()->all(), ['type' => 'sale'])) }}"
                        ...
                        <a href="{{ route('reports.accountant.download.csv', array_merge(request()->all(), ['type' => 'purchase'])) }}"
                        ...
                        <tbody class="text-sm text-slate-600 divide-y divide-gray-50">
                            @forelse($ventas as $venta)
                                <tr class="hover:bg-green-50/30 transition-colors">
                                    ...
                                    <td class="px-4 py-3">
                                        <span
                                            class="text-[10px] bg-blue-100 text-blue-700 px-2 py-1 rounded font-bold uppercase">
                            ...
                            @empty
                                <tr>
                                    <td colspan="5" class="px-4 py-8 text-center text-slate-500">No hay ventas registradas en
                                        este periodo</td>
                                </tr>
                            @endforelse
                        </tbody>
                        ...
                        <tbody class="text-sm text-slate-600 divide-y divide-gray-50">
                            @forelse($compras as $compra)
                                <tr class="hover:bg-red-50/30 transition-colors">
                                    ...
                                    <td class="px-4 py-3">
                                        <span
                                            class="text-[10px] bg-orange-100 text-orange-700 px-2 py-1 rounded font-bold uppercase">
                            ...
                            @empty
                                <tr>
                                    <td colspan="5" class="px-4 py-8 text-center text-slate-500">No hay compras registradas en
                                        este periodo</td>
                                </tr>
                            @endforelse
                        </tbody>
