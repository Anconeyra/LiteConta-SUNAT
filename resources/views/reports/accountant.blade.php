...
<div class="space-y-6">
    <div class="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
        ...
        <form action="{{ route('reports.accountant.generate') }}" method="POST" class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
            @csrf
            <div>
                <x-input-label for="start_date" value="Fecha Inicio" class="font-bold text-xs uppercase text-slate-400 mb-2" />
                <x-text-input id="start_date" name="start_date" type="date" class="w-full rounded-xl"
                              value="{{ request('start_date', now()->subMonths(3)->format('Y-m-d')) }}" required />
            </div>

            <div>
                <x-input-label for="end_date" value="Fecha Fin" class="font-bold text-xs uppercase text-slate-400 mb-2" />
            ...
            <div class="flex items-end gap-2">
                <button type="submit" class="flex-1 bg-green-500 text-slate-900 font-bold py-2.5 rounded-xl hover:bg-green-400 transition">
                    <i class="fas fa-filter mr-1"></i> Generar
                </button>
                <a href="{{ route('reports.accountant.download.pdf') }}?start_date={{ request('start_date', now()->subMonths(3)->format('Y-m-d')) }}&end_date={{ request('end_date', now()->format('Y-m-d')) }}"
                ...
                <table class="w-full text-left">
                    ...
                    <tbody class="text-sm text-slate-600 divide-y divide-gray-50">
                        ...
                        <tr class="hover:bg-green-50/30 transition-colors">
                            ...
                            <td class="px-4 py-3 text-right font-bold text-slate-800">S/ {{ number_format($venta->total, 2) }}</td>
                        </tr>
                        @empty
                        <tr>
                ...
                <table class="w-full text-left">
                    ...
                    <tbody class="text-sm text-slate-600 divide-y divide-gray-50">
                        ...
                        <tr class="hover:bg-red-50/30 transition-colors">
                            ...
                            <td class="px-4 py-3 text-right font-bold text-slate-800">S/ {{ number_format($compra->total, 2) }}</td>
                        </tr>
                        @empty
                        <tr>
