...
<div class="space-y-6">
    <div class="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 flex flex-col md:flex-row justify-between items-center gap-4">
        ...
        <a href="{{ route('compliance-alerts.create') }}" class="bg-green-500 text-slate-900 font-bold px-6 py-2.5 rounded-xl hover:bg-green-400 transition shadow-lg shadow-green-100">
                    ...
                    <tr class="hover:bg-slate-50 transition-colors">
                        ...
                        <td class="px-6 py-4 text-center">
                            <div class="flex justify-center gap-3">
                                <button onclick="openEditModal({{ $alert->id }})" class="text-slate-400 hover:text-blue-600 transition">
                                    <i class="fas fa-edit"></i>
                                </button>
                                <button onclick="openDeleteModal({{ $alert->id }}, '{{ addslashes($alert->title) }}')" class="text-slate-400 hover:text-red-500 transition">
        ...
        <form id="editForm" method="POST">
            ...
            <div class="p-6 space-y-4">
                <div>
                    ...
                    <input type="text" id="edit_title" name="title" class="w-full border-gray-200 rounded-xl focus:ring-green-500" required>
                </div>

                <div>
                    <label for="edit_description" class="block text-sm font-medium text-slate-700 mb-1">Descripción</label>
                    <textarea id="edit_description" name="description" rows="3" class="w-full border-gray-200 rounded-xl focus:ring-green-500"></textarea>
                </div>

                <div>
                    <label for="edit_alert_date" class="block text-sm font-medium text-slate-700 mb-1">Fecha de Alerta</label>
                    <input type="date" id="edit_alert_date" name="alert_date" class="w-full border-gray-200 rounded-xl focus:ring-green-500" required>
                </div>

                <div>
                    <label for="edit_notification_days_before" class="block text-sm font-medium text-slate-700 mb-1">Notificar con Anticipación (días)</label>
                    <input type="number" id="edit_notification_days_before" name="notification_days_before" min="0" max="365" class="w-full border-gray-200 rounded-xl focus:ring-green-500">
                ...
                <div>
                    <label class="inline-flex items-center">
                        ...
                        <input type="checkbox" id="edit_is_active" name="is_active" value="1" class="rounded text-green-600 focus:ring-green-500">
                        <span class="ml-2 text-sm text-slate-600">Activar alerta</span>
                    </label>
                </div>
            </div>
            <div class="p-6 border-t border-gray-100 flex justify-end gap-3">
                <button type="button" onclick="closeEditModal()" class="px-4 py-2 text-slate-600 font-medium rounded-xl hover:bg-slate-100">Cancelar</button>
                <button type="submit" class="bg-green-500 text-slate-900 font-bold px-6 py-2.5 rounded-xl hover:bg-green-400 transition">Actualizar</button>
            </div>
        </form>
    </div>
</div>
...
<div id="deleteModal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center hidden z-50 p-4">
    <div class="bg-white rounded-2xl shadow-xl w-full max-w-md">
        ...
        <div class="p-6">
            <p class="text-slate-600">¿Estás seguro de eliminar la alerta "<span id="deleteAlertTitle" class="font-bold"></span>"? Esta acción no se puede deshacer.</p>
        </div>
        <form id="deleteForm" method="POST">
            @csrf
            @method('DELETE')
            <div class="p-6 border-t border-gray-100 flex justify-end gap-3">
                <button type="button" onclick="closeDeleteModal()" class="px-4 py-2 text-slate-600 font-medium rounded-xl hover:bg-slate-100">Cancelar</button>
                <button type="submit" class="bg-red-600 text-white font-bold px-6 py-2.5 rounded-xl hover:bg-red-700 transition">Eliminar</button>
            </div>
        </form>
    </div>
</div>
...
<script>
    function openEditModal(alertId) {
        // Fetch alert data and populate the form
        fetch(`{{ route('compliance-alerts.show', '__id__') }}`.replace('__id__', alertId))
            ...
            .then(data => {
                if(data.success) {
                    ...
                    document.getElementById('edit_alert_date').value = alert.alert_date.split('T')[0];
                    ...
                    document.getElementById('editForm').action = `{{ route('compliance-alerts.update', '__id__') }}`.replace('__id__', alertId);
    ...
    function openDeleteModal(alertId, alertTitle) {
        ...
        document.getElementById('deleteForm').action = `{{ route('compliance-alerts.destroy', '__id__') }}`.replace('__id__', alertId);
