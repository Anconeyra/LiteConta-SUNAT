...
@section('content')
            ...
            <form action="{{ route('accounting.categories.update', $category) }}" method="POST">
                ...
                <div class="space-y-6">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            <x-input-label for="type" value="Tipo de Categoría"
                                class="font-bold text-xs uppercase text-slate-400 mb-2" />
                            <select id="type-select" name="type"
                                class="w-full border-gray-200 rounded-xl focus:ring-green-500" required>
                                <option value="income" {{ old('type', $category->type) == 'income' ? 'selected' : '' }}>Es un
                                    INGRESO (Entrada)</option>
                                <option value="expense" {{ old('type', $category->type) == 'expense' ? 'selected' : '' }}>Es
                                    un GASTO (Salida)</option>
                            </select>
                        </div>
                    </div>
                        ...
                        <div class="flex flex-col mb-4">
                            ...
                            <div class="flex gap-2">
                                <x-text-input id="accounting_code" name="accounting_code" type="text"
                                    class="w-40 rounded-xl bg-white font-mono font-bold {{ $category->type == 'income' ? 'text-green-700' : 'text-red-700' }}"
                                    value="{{ old('accounting_code', $category->accounting_code) }}" placeholder="Código"
                                    readonly />

                                <div class="flex-1 relative">
                                    <input type="text" id="pcge-search"
                                        class="w-full border-gray-200 rounded-xl focus:ring-blue-500"
                                        placeholder="Busca el concepto (ej: venta, luz, flete, planilla)...">
                            ...
                            <p class="text-[10px] text-slate-400 mt-2 italic text-center md:text-left">
                                Usa el buscador para cambiar el código PCGE si es necesario.
                            </p>
                        </div>
                    </div>
    ...
    <script>
        // La misma base de datos robusta para MYPEs en Perú
        const pcgeData = [
        ...
        function renderResults(data) {
            ...
            if (data.length === 0) {
                resultsDiv.innerHTML = '<div class="p-3 text-xs text-slate-500 italic text-center">No hay coincidencias.</div>';
            } else {
                data.forEach(item => {
                    ...
                    div.onclick = () => {
                        ...
                        // Ya no cambiamos el tipo automáticamente en EDIT para evitar confusión, 
                        // pero actualizamos el color del código
                        updateCodeColor(item.type);
                        resultsDiv.classList.add('hidden');
                        searchInput.value = ''; // Limpiamos búsqueda para que el usuario vea que ya seleccionó
                    };
        ...
        function updateCodeColor(type) {
            if (type === 'income') {
                ...
                codeInput.classList.remove('text-green-700');
        ...
        // Cambiar color inicial si el usuario cambia el SELECT manualmente
        typeSelect.addEventListener('change', () => {
            updateCodeColor(typeSelect.value);
        });
    </script>
@endsection